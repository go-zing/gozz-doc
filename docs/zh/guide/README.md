...
预处理阶段，`Gozz` 会收集出所有被注解的对象 `AnnotatedDecl`，并缓存相关的文件信息和 AST 解析信息在内存中。

在将注解对象正式分配给插件时，`Gozz` 会将所有被注解对象的注解，按行匹配出符合被调动插件的注解，此时每个注解对象都会被匹配到
0 ~ N 次。

被匹配的每条注解在解析为插件参数后，会和被注解对象组成一个新的对象 注解实例 `DeclEntity`，
每个插件的核心功能就是利用匹配到的若干个注解实例去进行不同的自定义代码分析处理。

在一次 `Gozz` 命令运行中，可以指定多个插件按顺序执行，如 `gozz run -p "api" -p "impl" ./`。

每个插件处理插件实例前，`Gozz` 都会重新对目录文件进行遍历解析，但由于之前的解析已经缓存文件的信息和解析数据，
文件没有被上一个插件修改，或新增文件时，这些缓存都会被有效复用。
